...
import {
  // createConnectionTag, getConnectionTagByValue,
...
const AssignTag = ({
  open, onClose, tags, onSetNotification,
  // onAddTag,
  userId, onAssignTags, connection,
}) => {
  ...
  // const [selectedTagColor, setSelectedTagColor] = useState('#999999')
  ...
  useEffect(() => {
    ...
    //   },
    // }))
    if (connection && connection.tags) {
      const adjustedTagsForm = adjustFormValues(tagsForm, connection, null)
  ...
  //   details.count = 0
  //   details.color = selectedTagColor
  //   try {
  ...
  const assignTagsHandler = async e => {
    ...
    const tagsDisplay = tags.filter(opt => tagsDetails.tags.includes(opt.id)).map(opt => opt.display)
    ...
    try {
      await assignConnectionTags(userId, connection.id, tagsDetails.tags, tagsDisplay)
      onAssignTags(connection.id, tagsDetails.tags, tagsDisplay)
    ...
    >
      <Box className={classes.viewCardData}>
        ...
        <Grid container spacing={3} className={classes.formTagsContainer}>
          ...
          {/* <Grid item xs={12}>
            <Box className={`${layoutClasses.panel} ${classes.addNewTagBoxContainer}`}>
              <PageTitle title={pageStatics.data.titles.assignTagDialogAddNewTagPanel} />
              <Box mb={2}>
                <Grid container spacing={3} alignItems="center">
                  {loadTagsForm(addTagForm)}
                  <Grid item xs={12}>
                    <Box mb={2}>
                      <Typography component="p" variant="body1" className={layoutClasses.panelText} style={{ fontWeight: 400, marginBottom: 8 }}>
                      ...
                      <Box className={classes.labelColorsContainer}>
                        {labelColors.map(labelColor => (
                          <Button
                            key={labelColor.name}
                            color="secondary"
                            onClick={() => changeLabelColorHandler(labelColor)}
                            className={classes.labelColorButton}
                            style={{
                              backgroundColor: labelColor.code,
                            }}
                          >
                            {labelColor.code === selectedTagColor && (
                              <CheckIcon />
                            )}
                          </Button>
                        ))}
...
const mapDispatchToProps = dispatch => ({
  ...
  onAssignTags: (connectionId, assignedTags, assignedTagsDisplay) => dispatch(actions.assignConnectionTags(connectionId, assignedTags, assignedTagsDisplay)),
})

export default connect(null, mapDispatchToProps)(AssignTag)
